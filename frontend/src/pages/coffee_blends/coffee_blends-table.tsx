...
import {setRefetch, uploadCsv} from '../../stores/coffee_blends/coffee_blendsSlice';
...
const Coffee_blendsTablesPage = () => {
  ...
  const [filters] = useState([{label: 'Name', title: 'name'},
    ...
    const getCoffee_blendsCSV = async () => {
        const response = await axios({url: '/coffee_blends?filetype=csv', method: 'GET',responseType: 'blob'});
  ...
  return (
    <>
      ...
      <SectionMain>
        <SectionTitleLineWithButton icon={mdiChartTimelineVariant} title="Coffee_blends" main>
        {''}
        </SectionTitleLineWithButton>
        <CardBox className='mb-6' cardBoxClassName='flex flex-wrap'>
          <BaseButton className={'mr-3'} href={'/coffee_blends/coffee_blends-new'} color='info' label='New Item' />
          ...
          <BaseButton
              ...
              onClick={addFilter}
          />
          <BaseButton className={'mr-3'} color='info' label='Download CSV' onClick={getCoffee_blendsCSV} />
            <BaseButton
              color='info'
              label='Upload CSV'
